...
- **格式工具栏与快捷键** - Cmd/Ctrl+B（粗体）、I（斜体）、U（下划线）、K（链接）、Z（撤销，50 步历史）、F（查找替换）
- **查找替换** - Cmd/Ctrl+F 呼出双行查找栏，支持区分大小写与正则（含 `$1` 捕获组），编辑区高亮全部匹配
- **专注工作区** - 可在双栏、仅编辑和仅预览间切换
...
- **推送草稿箱** - 配置公众号 AppID/AppSecret 后直接推送到草稿箱，标题、封面与摘要自动读取 frontmatter（摘要取 `description`，上限 120 字）；封面 webp/svg 自动转 png；凭证仅保存在浏览器本地，多用户互不干扰
