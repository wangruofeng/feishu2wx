...
- 🎨 **5 种精美主题** - 内置 5 种主题（经典、橙色、蓝色、青绿、自定义），自定义主题在设置面板挑选一个主色即可生成整套配色；支持浅色/深色/跟随系统三种模式
- ⚙️ **排版设置面板** - 可视化设置面板，按通用 / 标题 / 正文 / 引用块 / 图片 / 代码块 / 模板 / 公众号分组管理排版选项
...
- 🧩 **文章首尾模板** - 在设置面板配置首尾模板，复制/推送时自动拼接到正文前后（frontmatter 之后、正文之前 / 正文之后），可单独开关首/尾部片段是否插入
...
- 💾 **导出 HTML** - 将排版后的内联样式 HTML 导出为独立 `.html` 文件，浏览器双击即可预览，文件名自动取文章标题
...
- 🌈 **编辑器源码高亮** - Markdown 编辑器内置语法高亮（GitHub / Dracula / Monokai / 无 配色），在不影响输入的前提下着色标记
...
- 🤖 **AI 聊天编辑** - 在 AI 助手面板发送指令修改当前文章，支持图片/文本附件与流式输出，可先「查看变更」再「应用修改」；面板支持抽屉浮层 / 右侧侧栏两种显示方式（设置面板可切换）
- ☁️ **AI 供应商云端配置** - 自定义模型供应商（Chat Completions / Anthropic / Responses 三种 API 格式）；Cloudflare 部署版可用 GitHub 登录，配置 AES-GCM 加密同步到云端，浏览器端不暴露 API Key
...
4. **切换主题**
    ...
    - 点击顶部主题选择器（位于第二行工具栏）
    - 选择您喜欢的主题风格（5 种主题可选，自定义主题色在设置面板中配置）
...
15. **复制到公众号**
    ...
    - 设置面板开启“微信公众号链接自动适配”后，Markdown 链接会转换为“链接文字: URL”普通文本格式；关闭后保留超链接
...
16. **导出 HTML 文件**
    ...
    - 点击"导出"按钮，将排版后的内联样式 HTML 下载为独立 `.html` 文件
    - 文件名自动取文章标题（front matter `title` 或正文首个 H1）
    - 下载的文件可在浏览器中直接打开预览排版效果，样式与复制到公众号完全一致
...
- **代码块语法高亮** - 支持多种编程语言的语法高亮，使用 Atom One Dark 主题；极简和现代风格均采用清晰的等宽字体
...
- **引用块优化** - 上下 padding 保持一致，并支持独立配置背景、边框色、间距（宽松/紧凑）
...
在「排版设置面板 → 通用」中挑选任意主色，即可自动生成整套配色（标题、引用块、链接等随之适配）；清空输入并点击「恢复预设」可回到预设主题。
...
支持单次命令覆盖主题与排版项（如 `--theme blue --invert-h1 --text-align-mode justify`）。CLI 只读取用户级 `~/.feishu2wx/config.json`；凭证不应放入项目目录或提交到仓库。完整命令速查与各项说明见 [cli/README.md](cli/README.md)。
