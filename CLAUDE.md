...
- AI 聊天编辑：fab-group「AI」按钮打开右侧非模态抽屉（`src/components/ai/`），通过 `POST /api/ai/chat` 让 AI 修改当前文章；后端双轨共用 `server/lib/ai-handler.ts`（三格式上游代理 anthropic/chat-completions/responses + SSE 归一化为 reasoning/delta/error 事件），Express 壳 `server/routes/ai.ts`、CF 壳 `functions/api/ai/chat.ts`；协议为「说明文字 + `<<<ARTICLE … ARTICLE>>>` 全文修改稿」（`createAiStreamParser` 状态机处理标记被 chunk 拆断），前端流式解析在 `src/utils/aiChat.ts`、diff 在 `aiDiff.ts`、气泡 Markdown 用独立的 `aiMarkdown.ts`（不复用 `renderMarkdown`）；附件统一入口为输入框左侧「+」按钮：图片（粘贴/拖拽/选择，≤6 张）走 `aiImages.ts` 多模态 base64 管线，文本附件 md/csv/json 等（`aiFiles.ts`，单文件 ≤200KB）作为 `attachments` 随请求发送、由服务端 `buildAttachmentsBlock` 以「文件名+围栏代码块」注入用户消息文本（不依赖多模态），持久化剔除图片与附件内容（保留文件名）；供应商配置（含 API Key）存 localStorage 键 `feishu2wx_ai_config` 随请求体发送（BYOK，服务端零存储），聊天历史 `feishu2wx_ai_messages`（20 条，持久化剔除图片）、输入历史 `feishu2wx_ai_input_history`；应用修改走 `EditorPane` 的 `forwardRef` 暴露的 `replaceWholeDocument()`（存历史文档 → 进撤销栈 → setMarkdown，Cmd+Z 可撤销）；GitHub Pages 纯静态部署无此端点，前端有降级提示；设计细节见 [docs/superpowers/specs/2026-08-29-ai-chat-editor-design.md](docs/superpowers/specs/2026-08-29-ai-chat-editor-design.md)
