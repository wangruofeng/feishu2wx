...
- `src/components/ui/Button.tsx`：统一按钮组件，所有 `<button>` 应从该组件导入，通过 `variant` 映射原始 CSS 类名；`src/components/ui/GearIcon.tsx` 为共享设置齿轮图标（Lucide 线性齿轮），顶栏设置按钮与 AI 助手头部「模型设置」按钮复用同一组件，`size` 属性按场景调整
