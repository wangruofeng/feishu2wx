...
- 主题配置分散在三处（`ThemeSwitcher.tsx`、`wechatCopy.ts`、`styles/themes.css`），ThemeSwitcher 深色模式色值在 `ThemeSwitcher.css` 中独立控制，不通过 CSS 变量继承；自定义主题色（localStorage 键 `feishu2wx_customThemeColor`）由 `src/utils/themeColor.ts` 的 `buildCustomThemePalette()` 从单一主色推导完整色板（非法/空值回退 `#0d9488`），预览层由 `App.tsx` 在根节点注入 `--custom-*` CSS 变量、`themes.css` 的 `.theme-custom` 规则消费，公众号输出层由 `wechatCopy.ts` 的 `getThemeStyles()` 消费
