...
- 拖拽导入与历史文档：`.md` 文件可拖到编辑区导入（与「📂 导入」按钮共用 `importMarkdownFile()`，替换前旧内容自动存历史 + 进撤销栈）；历史存取在 `src/utils/docHistory.ts`（localStorage 键 `feishu2wx_docHistory`，上限 20 条、单份超 200KB 跳过），触发点为导入/拖拽/清空/加载示例/历史恢复之前，全部在 `EditorPane` 内触发；浮层组件 `DocHistoryPopover.tsx`，与大纲浮层互斥、复用底栏定位逻辑
