...
- 拖拽导入与历史文档：`.md` 文件可拖到编辑区导入（与「📂 导入」控件共用 `importMarkdownFile()`，替换前旧内容自动存历史 + 进撤销栈）；图片附件（含 SVG）拖拽/粘贴到编辑区插入为 data URI 图片（见 SVG 识别链路条目）；历史存取在 `src/utils/docHistory.ts`（localStorage 键 `feishu2wx_docHistory`，上限 20 条、单份超 200KB 跳过），触发点为导入/拖拽/清空/加载示例/历史恢复之前，全部在 `EditorPane` 内触发；浮层组件 `DocHistoryPopover.tsx`，与大纲浮层互斥、复用底栏浮层定位逻辑；隐藏的 file input 必须用视觉隐藏（`position:absolute; opacity:0`）而非 `display:none`，且「📂 导入」通过 `<label for>` 原生关联该 input，避免 Safari/WebKit 阻止脚本 `click()` 打开文件选择框
...
- 导出文件：`App.tsx` 顶栏「导出」按钮为下拉菜单（`ExportMenu.tsx`），支持 md / html / pdf 三种格式；md 直接下载 `composedMarkdown`，html 复用复制/推送同款管线（`convertSvgImagesToPng` → `formatForWeChat`）后由 `wechatCopy.ts` 的 `exportHtmlToFile()` 下载，pdf 经 `exportHtmlToPdf()` 载入隐藏 iframe 调用浏览器打印对话框「另存为 PDF」；文件名均取 `articleTitle`
