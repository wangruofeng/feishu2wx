...
若 GitHub 授权回调失败，重新发起登录获取新的授权码；授权码不可复用。授权码或 OAuth App 凭据不匹配会返回可读的 400 提示（不用 502，避免 Pages 替换错误正文）；回调返回“登录服务配置无效”时，检查上述 Secret 是否配置在 **Production** 环境，且两项随机密钥均为独立的 32 字节 base64url 值。
